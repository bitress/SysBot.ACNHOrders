...
namespace SysBot.ACNHOrders
{
    public class RemoteControlInteractionModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        public async Task ClickAsync(SwitchButton button)
        {
            ...
            await RespondAsync($"{b.Connection.Name} has performed: {button}");
        }

        [SlashCommand("setstick", "Sets the stick to the specified position.")]
        [RequireSudoInteraction]
        public async Task SetStickAsync(SwitchStick stick, short x, short y, int ms = 1_000)
        {
            ...
            await RespondAsync($"{b.Connection.Name} has performed: {stick}");
            ...
            await FollowupAsync($"{b.Connection.Name} has reset the stick position.");
        }

        [SlashCommand("readmemory", "Reads memory from the requested offset and writes it to the bot directory.")]
        [RequireSudoInteraction]
        public async Task ReadMemoryAsync(uint offset, int length)
        {
            var b = Bot;
        ...
        [RequireSudoInteraction]
        public async Task ReadCommandAsync(int size, string command)
        {
            var b = Bot;
            var data = System.Text.Encoding.UTF8.GetBytes(command + "\r\n");
            await RespondAsync($"Sending `{command}` and waiting for {size}-byte result.");
            var ret = await b.SwitchConnectedConnection.ReadRaw(data, size, CancellationToken.None).ConfigureAwait(false);
            await FollowupAsync($"`{command}` returned with result: {System.Text.Encoding.UTF8.GetString(ret)}");
        }
        ...
        public async Task UnfreezeAll()
        {
            ...
            await RespondAsync("Unfrozen all previously frozen values");
        }
        ...
        public async Task SetFreezeDelay(int ms)
        {
            ...
            await Bot.SwitchConnectedConnection.SendRaw(data, CancellationToken.None).ConfigureAwait(false);
            await RespondAsync($"Set freeze rate to: {ms}");
        }
        ...
        public async Task FreezePause()
        {
            ...
            await RespondAsync("Freeze has been paused.");
        }
        ...
        public async Task FreezeUnpause()
        {
            ...
            await RespondAsync("Freeze has been unpaused.");
        }
