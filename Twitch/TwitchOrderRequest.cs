    ...
    public class TwitchOrderRequest<T> : IACNHOrderNotifier<T> where T : Item, new()
    {
        ...
        public void OrderReady(CrossBot routine, string msg, string dodo)
        {
            ...
            if (Settings.OrderWaitDestination == TwitchMessageDestination.Channel)
                SendMessage($"I'm waiting for you @{Trader}! {msg}. Enter the number you whispered to me on https://www.chopaeng.com/dodo?hash={SimpleEncrypt.SimpleEncryptToBase64(dodo, Password).MakeWebSafe()} to get your dodo code. Click this link, not an old one or someone else's.", Settings.OrderWaitDestination);
